#include "ps2_runtime_macros.h"
#include "ps2_runtime.h"
#include "ps2_recompiled_functions.h"
#include "ps2_recompiled_stubs.h"

#include "ps2_syscalls.h"
#include "ps2_stubs.h"

#ifdef PS2_FUNCTION_LOG_TRACKER
#include "ps2_log.h"
#endif

// Function: sub_001422C0
// Address: 0x1422c0 - 0x1422d0
void sub_001422C0_0x1422c0(uint8_t* rdram, R5900Context* ctx, PS2Runtime *runtime) {
#ifdef PS2_FUNCTION_LOG_TRACKER
    PS_LOG_ENTRY("sub_001422C0_0x1422c0");
#endif

    ctx->pc = 0x1422c0u;

    // 0x1422c0: 0x80282d  daddu       $a1, $a0, $zero
    ctx->pc = 0x1422c0u;
    SET_GPR_U64(ctx, 5, (uint64_t)GPR_U64(ctx, 4) + (uint64_t)GPR_U64(ctx, 0));
    // 0x1422c4: 0x3c04006c  lui         $a0, 0x6C
    ctx->pc = 0x1422c4u;
    SET_GPR_S32(ctx, 4, (int32_t)((uint32_t)108 << 16));
    // 0x1422c8: 0x804cce8  j           func_1333A0
    ctx->pc = 0x1422C8u;
    ctx->pc = 0x1422CCu;
    ctx->in_delay_slot = true; ctx->branch_pc = 0x1422C8u;
            // 0x1422cc: 0x24845a30  addiu       $a0, $a0, 0x5A30 (Delay Slot)
        SET_GPR_S32(ctx, 4, (int32_t)ADD32(GPR_U32(ctx, 4), 23088));
        ctx->in_delay_slot = false;
    ctx->pc = 0x1333A0u;
    if (runtime->hasFunction(0x1333A0u)) {
        auto targetFn = runtime->lookupFunction(0x1333A0u);
        targetFn(rdram, ctx, runtime); return;
    } else {
        sub_001333A0_0x1333a0(rdram, ctx, runtime); return;
    }
    ctx->pc = 0x1422D0u;
}
